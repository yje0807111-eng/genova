...
export const adminTokens = {
  ...
  // 입력
  input:
    "h-9 rounded-md border border-white/[0.08] bg-[#080618] px-3 text-[13px] text-white placeholder:text-white/30 focus:border-white/[0.2] focus:outline-none transition-colors",
  inputLabel: "text-[11px] font-medium text-white/50 mb-1.5",

  // 버튼
  buttonPrimary:
    "h-8 px-3 rounded-md bg-white text-[#080618] text-[12px] font-semibold hover:bg-white/90 transition",
  buttonSecondary:
  ...
  // 아이콘 버튼 (정사각형)
  iconButton:
    "h-8 w-8 rounded-md flex items-center justify-center text-white/40 hover:text-white hover:bg-white/[0.06] transition",

  // 배지
  badge: "inline-flex items-center gap-1 px-2 py-0.5 rounded text-[10px] font-medium uppercase tracking-wider",
  badgeNeutral: "bg-white/[0.06] text-white/60",
  ...
  // 테이블
  tableHeader: "text-[10px] font-semibold uppercase tracking-[0.12em] text-white/40 px-3 py-2",
