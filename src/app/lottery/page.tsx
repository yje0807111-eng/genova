...
/**
 * 사용자(크리에이터)용 응모권 시스템 안내 페이지.
 *
 * 운영 핸드북(docs/lottery-operator-handbook.md)의 규칙을 사용자
 * 언어로 풀어쓴 정적 가이드.  진입점: 응모권 카운터 ⓘ / 콘테스트
 * 상세 / 푸터.
 *
 * 정적 콘텐츠가 많아 i18n 키로 빼면 키가 폭증하므로, 페이지 로컬
 * 콘텐츠 객체를 locale 로 분기한다 (translations.ts 비오염).
 * 서버 컴포넌트 유지 — FAQ 는 네이티브 <details> 아코디언.
 */
...
const CONTENT: Record<Locale, LotteryContent> = {
  ko: {
...
export default async function LotteryGuidePage() {
  ...
  return (
    <div className="mx-auto max-w-[920px] px-6 pb-20 pt-10 sm:px-8">
      {/* Hero */}
      <div className="text-center">
        ...
        <h1
          className="mt-3 bg-clip-text text-2xl font-black leading-tight text-transparent sm:text-[34px]"
          style={{ backgroundImage: "linear-gradient(135deg, #fff 0%, #AFA9EC 100%)" }}
        >
          {c.title}
        </h1>
        <p className="mx-auto mt-4 max-w-[640px] text-[14px] leading-relaxed text-white/55">
          {c.subtitle}
        </p>
      </div>

      {/* Steps */}
      <h2 className="mt-14 mb-5 text-[13px] font-bold uppercase tracking-[0.18em] text-white/45">
        {c.stepsTitle}
      </h2>
      ...
      {/* Eligibility */}
      <h2 className="mt-14 mb-4 text-[13px] font-bold uppercase tracking-[0.18em] text-white/45">
        {c.eligTitle}
      </h2>
      ...
      {/* FAQ — native details accordion (server component safe) */}
      <h2 className="mt-14 mb-4 text-[13px] font-bold uppercase tracking-[0.18em] text-white/45">
        {c.faqTitle}
      </h2>
